...
static const char *transport_default_values[] = {
	...
	[TRANSPORT_VAR_PLAY_MEDIUM] = "SPDIF",// "UNKNOWN",
	[TRANSPORT_VAR_REC_MEDIUM] = "NOT_IMPLEMENTED",
	[TRANSPORT_VAR_PLAY_MEDIA] = "SPDIF,UNKNOWN",// "NETWORK,UNKNOWN",
...
static int divide_leave_remainder(int *val, int divisor) {
	int result = *val / divisor;
	*val %= divisor;
	return result;
}
static void print_upnp_time(char *result, size_t size, int t) {
...
static int parse_upnp_time(const char *time_string) {
	int hour = 0;
	int minute = 0;
	int second = 0;
...
static void *thread_update_track_time(void *userdata) {
	char tbuf[32];
	int last_duration = -1, last_position = -1;
	for (;;) {
		usleep(500000);  // 500ms
		service_lock();
...
static void update_track_time(void) {
	char tbuf[32];
	int duration, position;

	if (output_get_position(&duration, &position) == 0) {
		service_lock();
...
void upnp_transport_init(struct upnp_device *device) {
	...
	transport_service_.last_change = UPnPLastChangeCollector_new(state_variables_, TRANSPORT_EVENT_XML_NS, device, TRANSPORT_SERVICE_ID);
